...
stmt:
  ...
  | e1 = expr ASSIGN e2 = expr {Sassign(e1, e2, None)}
...
iterative:
  | FOR id = ident ASSIGN e1 = expr TO e2 = expr LBRACE s = suite RBRACE { 
    Sfor(id, e1, e2, s, 1) }
...
collections:
  | id = ident LPAREN p = expr_list RPAREN LBRACE s = suite RBRACE { 
    Sdef(id, p, s) }
  | LET id = ident BE A NEW list = ident { Snewlist(id, Ecst(Cnil, None), list) }
  | LET id = ident LBRACKET RBRACKET BE A NEW list = ident { Snewlist(id, Ecst(Cnil, None), list) }
...
expr:
  | e1 = expr o = binop e2 = expr { Ebinop(o, e1, e2, None) }
  | LPAREN e1 = expr COMMA el = expr_list RPAREN { Etuple(e1 :: el, None) }
  | id = ident LPAREN args = expr_list RPAREN { 
    ignore(add_ident (string_of_ident id) (Expr (Ecall(id, args, None))) !current_scope_id);
    Ecall(id, args, None) }
  | e1 = expr DOT e2 = ident { Eattribute(e1, e2, None) }
  | LBRACKET e = expr_list RBRACKET { Elist(e, None) }
  | id = ident LBRACKET e = expr RBRACKET { Eget(id, e, None) }
  | s = simple_expr { s }
;

simple_expr:
  | u = unop e = expr { Eunop(u, e, None) }
  | LPAREN e = expr RPAREN { e }
  | e1 = ident { Eident(e1) }
  | c = constant { Ecst(c, None) }
;
...
ident:
  id = ID { { loc = ($startpos, $endpos); id; typ = None } }
;
